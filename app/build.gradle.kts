...
// 与 BZK 同一套签名：keystore.properties + app-sign.keystore（都不入库）
...
android {
    namespace = "moe.lovefirefly.bzk.gboardext"
    compileSdk = 37

    defaultConfig {
        applicationId = "moe.lovefirefly.bzk.gboardext"
        minSdk = 27
        targetSdk = 36
        versionCode = 1
        versionName = "1.0.0"
    ...
    onVariants { variant ->
        variant.outputs.forEach { output ->
            (output as VariantOutputImpl).outputFileName.set(
                "BetterZUIKey-GboardExt-v${output.versionName.get()}.apk"
            )
        }
    }
}
