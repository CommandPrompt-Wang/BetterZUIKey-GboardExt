...
final class SymbolNorm {
     ...
     *
     * <p>不含任何中文标点（{@code ！？；：，（）＂} 这些在中文里本来就该是全角），
     * 也不含反引号与下划线（那两个走语义层，见 {@link #apply}）。
     *
     * <p><b>2026-09 修正</b>：早先这里写"刻意不含 {@code ＋＝}（实测那两个本来就正常）"——
     * 用户实测<b>不对</b>，{@code +} / {@code =} 键在中文态同样被 Gboard 打成全角。
     * 现在与 {@code ＠＃％＆＊～} 同一条规则补回；顺带把同族的 {@code ＄＾} 一起补上
     * （都是"ASCII 可打印符号被全角化"这一类）。查过 Gboard 的 dex：里面<b>没有</b>
     * 任何全角符号表（{@code ＋＝｛＠} 一个都不出现），说明它的全角化不是查表而是
     * 统一变换 ⇒ 靠"逐个补表"终归会漏，真要彻底得改成区间规则（见 plan.md P2 的宽度层）。
     */
