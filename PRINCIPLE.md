...
用自写 dex 扫描器对比三代 Gboard，**混淆名整批更换，而本模块的挂点一个都没动**：
...
这一节是本项目最曲折的部分，**三轮结论互相推翻**，完整记录下来以免重走弯路。
...
这个 binder 的**代理位于调用方进程内** ⇒ 挂钩它等同于"只拦截 Gboard 自己发起的调用" ✓。

### 第三轮：一次采样事故（教训比结论更有价值）

用户反馈"Ctrl+Space 被屏蔽了"，但日志里 `strict: blocked` 为 **0 次** ⇒ 据此判断"不是本模块所为" ✗。
用户**停用模块后 Ctrl+Space 立刻恢复** ⇒ 确为本模块拦截。

根因是**采样**：测试前后均执行 `logcat -c`，而那些 `blocked` 行恰好落在被清掉的窗口内。

> **教训**：清日志要么放在整个测试之前，要么不清。不要让采样条件掩盖事实。

顺带纠正了一个直觉：**输入法有焦点时，按键先进输入法窗口** ⇒
...
所以严格模式开启时 BZK 仍能正常切换语言。这是设计上必然成立的结果，并非巧合。
...
符号归一的**门控**是"当前是不是中文态"。该状态的来源经过三次修正才收敛。
...
| 三 | **日语被改** | 严格模式拦截了 Gboard 的 `switchInputMethod` ⇒ 框架**无从得知**语言已变，`IMM.getCurrentInputMethodSubtype()` 给的是**过期的 zh** |
...
严格模式拦截 `switchInputMethod` 后，框架一直认为仍在中文。于是**只要更换输入框 / 切换 App**，
`onStartInput` 触发的那次刷新就会把刚学到的 `ja-JP` 覆盖回 `zh-CN` ⇒ 日语符号被归一半角 ✗。

**修法**（`refreshLang`）：记住上次从框架读到的 hash，**hash 未变即不允许覆盖** ——
...
> **"认不出来" ≠ "不是中文"。** 早期在认不出来时清空状态，结果连中文也一并关掉。
...
用户观察：**软键盘的全半角正确，只有物理键盘出错** —— 这是一个"物理键盘特供 bug"。

证据链：

1. Gboard 的 4 个 dex 里**没有任何全角符号串**（`＋＝｛＠` 全 0 命中，也没有 `Halfwidth-Fullwidth` /
   `65248` / `0xFEE0`）。⚠️ 但**不能据此得出"不是查表"** —— 这个结论曾被推翻一次。
...
- 最初采用"逐个补表"（11 → 15 对），结果漏了 `＋＝`（用户反馈）—— 因为原生表覆盖**整个**
  ASCII 可打印区，**补表如同打地鼠**。
- 值得庆幸：区间规则对软键盘**天然是 no-op** ⇒ **不需要区分"提交来自软键盘还是物理键盘"** ✓

### 全角化的范围：提交含字母数字，组合态只动符号

宽度层只有一个 `fullWidth(src, withAlnum)`，两条入口给不同的 `withAlnum`：

| 入口 | 转哪些区间 | 结果 |
|---|---|---|
| `commitText` → `toFullWidth`（`withAlnum=true`） | `0x21–0x2F` / `0x30–0x39` / `0x3A–0x40` / `0x41–0x5A` / `0x5B–0x60` / `0x61–0x7A` / `0x7B–0x7E` | 符号 + **字母数字**（`ＡＢＣ１２３`） |
| `setComposingText` → `toFullWidthSymbols`（`withAlnum=false`） | 只碰符号那四段 | **只转符号**，字母数字原样 |

> 组合态为什么不能连字母一起转：中文态下 `setComposingText` 拿到的就是**拼音串**（`nihao`），
> 转了会变成全角拼音 ｎｉｈａｏ，非常不美观；而组合态里分不出"拼音"和"正在输入的英文单词"
> （两者都是纯 ASCII 字母），所以一律只动符号 —— 等到 `commitText` 时再按用户口径
> （"全角"就该包含字母数字 ＡＢＣ１２３）一起转。
> 与搜狗那边"整段 ASCII 都全角化"的**有意差异也只在组合态**。
...
| 本模块要做的事 | 把 `·` **还原**成 ASCII 反引号（本模块的覆盖） | 把 `｀` **改成** `·` |
...
拼音栏还有字时按 Enter 想保留原始拼音，结果整个输入框被提交。
...
2. 把 Enter 的 **down 与配对的 up 都补 `META_SHIFT_ON`** 交给 Gboard ⇒ 它走自己的 Shift+Enter
   路径（拼音正常上屏、注入的抬起带 Shift）。
3. 再把这次"给拼音上屏用的"注入事件**吞掉** ⇒ 对"只认注入事件"的输入框兜底。
4. **不自己 commit 拼音** —— 交给 Gboard，它引擎内部状态才同步（手动 commit 有脱同步风险）。

> **踩坑：up 必须 sticky。** down 之后 Gboard 的 `commitText` 已把 composing 清成 `false`，
> 只按 composing 门控就会漏掉 up ⇒ 注入的抬起不带 Shift ⇒ **仍会被提交** ✗。
> 另外拼音栏为空时要清掉"吞注入"旗标，以免残留状态把后面真正应当提交的那一次也吞掉。

### 已知边界

- **软键盘**上的 Enter 不走 `onKeyDown`，不覆盖。
- **QQ 消息栏**：IME 侧**无解**。针对它所做的那轮测试里 `ENTER key seen` 一行都没有
  （同进程 Edge 有）⇒ 说明 QQ 在**自己进程内**就消费了回车、根本不交给 IME
  （按 Enter 后反复重开会话 = "发送 + 清空 + 重新聚焦"）。"补 Shift"与"吞注入"对它均无效。
  日志判据即：一行都没有 ⇒ 无需在 IME 侧继续排查。
...
- 自检：清洗后长度必须**偶数**（奇数会导致最后一对被切分错误，而解析是**静默丢弃**的）。
...
光标后侧**已经是同一个闭字符** ⇒ 只把光标移过去，不再额外插入一个。

- 记录的是"**刚补出的是哪个字符**"，不是偏移 —— 偏移会被 `moveCursorLeftOne`、输入法自己、
  以及**用户点击**改掉，记录了也会很快失效。
- 用户**手动把光标点到别处** ⇒ 上次补全作废（靠 `onUpdateSelection` 区分"用户点的"与
  "本模块自身挪动的"）。**注意这条依赖 §7 的挂点修正**。

### 两个"静默失败"型的坑

1. **配对表根本没有发出去**：把发送逻辑从 `MainActivity` 挪进 `ConfigSender` 时，补 extra 的
   字符串替换**锚点未匹配上**（构建仍然通过）⇒ `EXTRA_PAIR_TABLE` 从未发送 ⇒ 模块收到 `null`
   ⇒ 解析出**空表** ⇒ 永远不配对。
...
## §7 两个"钩子静默失效"的坑（跨版本必修）

**Xposed 挂的是「方法」，不是「虚分派」。** Gboard 覆盖了某框架方法且**不调 `super`**
⇒ 框架实现永不执行 ⇒ **挂在框架类上的那条钩子从不会触发**（不报错、不崩溃，只是静默失效）。

静态核查（三代同形，所以不是新版回归）：

| 框架方法 | Gboard 是否覆盖 | 覆盖版调 `super`？ | 本模块的钩子 |
|---|---|---|---|
...
| `onStartInput` / `onStartInputView` | 是 | **否** | **死**（开局的 `refreshLangAsync` 失效） |
| `onCreateInputView` | 是 | **否** | **死**（`Banner.attachView` 靠 `setInputView` 兜底） |
...
**跳过框架类本身**；挂到 ≥1 个就把标志置上，框架那份退化为纯放行（以免调 `super` 的机型回调两遍）。
...
| 分析提交 lambda（`Lmn` / `Lmza`） | 是 **R8 合并出的 lambda 分发器**：一个类塞入几十个 lambda 体，`run()` 用字段 `d` 做 switch；构造点被**所有** lambda 共用 ⇒ hook 它无法区分哪个键，还会被全 Gboard 的 lambda 大量干扰 ✗ |
...
| **移除 `/` 档，只留 `\`（默认）与「全部」** | 两者都是**纯字符级**、可精确实现 ✓ |

⇒ 最终**删除**了顿号映射（用户决定）。DexKit 的**结构定位能力留在代码里**（打开 `DEV_INPUT_TRACE`
即用），以后需要时不必重走这段。
...
## §9 配置通道：为什么使用广播（而不是 provider）

沿用 SogouOEMExt 的设计做了 SharedPreferences + ContentProvider + 2 秒轮询，**在 Gboard 上失败**，
根因**不是权限而是包可见性**：
...
- Gboard 的 `targetSdk = 36` ⇒ Android 11+ 的**包可见性过滤**生效 ⇒ 它**看不到本模块的包** ✗
- SogouOEMExt 能生效是因为**搜狗的 targetSdk 是 29**（不过滤）
...
包可见性只约束**发起方** ⇒ 反过来由本模块（能看到 Gboard）发送**显式广播**即可送达；
接收端在 Gboard 进程内用输入法服务的 Context **运行时注册**
（`RECEIVER_EXPORTED`，targetSdk 34+ 必须显式声明），**不要求宿主 APK 声明任何内容**。

> **更正一条早期结论**：一度以为"发给别人也得能看见它"，于是在清单里加了
> `<package android:name="com.google.android.inputmethod.latin"/>` 的 `<queries>`。
> 后来实测**不需要**：把整个 `<queries>` 删掉，App 发的显式广播仍然能送到 Gboard ✓。
> 包可见性拦的是"查 / 拉起别人的组件"，**不拦发送广播**。
>
> **但 `<queries>` 仍然留着**，因为另一件事需要它：严格模式要 `getPackageInfo` **探测 BZK 是否已安装**
> （不声明会被拦截 ⇒ 误报"未检测到"）。

### 配置落盘到「目标进程」

广播是"一次性"的：Gboard 进程一重启就回到编译期默认值，开关会**静默恢复默认值**
（`enterCommitPinyin` 尤其明显：默认关 ⇒ 用户明明已打开却会间歇性失效）。

⇒ 模块收到广播时一并将配置写入**目标进程自己的 prefs**（`gboardext_state`），启动时先读它作为初值。

### 补播链（因为接收器只在 Gboard 活着时存在）

"修改设置的时刻"Gboard 通常并未运行（用户先拨开关，之后才去打字）⇒ App 侧安排一条**有界**重试链：
...
差几分钟无妨 —— 目标是"覆盖用户接下来这半小时内的第一次打字"。

> **实测边界**：这条链在**这台 ROM** 上只能覆盖"我方 App 进程还活着"的窗口 —— ZUI 的自启动管理
> 拒绝为广播冷启动我方进程（`ZuiAutoRunManager: isAllowStart not allow start ...`），
> 且设备上还有 Thanox 在跑。彻底覆盖要在 ROM 侧放行自启动；**保底做法是在键盘开启状态下打开一次 App**。
> 这也是设置页里那个「自启动权限」入口存在的原因。

### 状态位为什么走「反向广播」

三个状态位存在 **Gboard 进程**的 prefs 里，App **无法读取** ⇒ 设置页只能显示"这个功能是否启用"、
无法显示"当前是哪一档"。所以热键切换的那一刻，模块发一条**显式指定包名**的广播回去
（不指定包名会被包可见性丢弃）；App 收到后落盘到镜像文件，进入页面时先读它。
...
代价：单独按一下再松开 Shift 仍然正常切换语言（这才是用户期望的行为）。

### 提示用横幅，不用 Toast

搜狗那边实测 `Toast` 会被系统按**应用通知设置**拦截
（`NotificationService: Suppressing toast ... by user request`），所以沿用同一套做法：
在输入法窗口上加一个 `PopupWindow`，贴底居中、约占屏幕高 12%、1.2 秒后消失。
视图在 `setInputView` / `onCreateInputView` 时由 `ServiceProbe` 挂上。
...
触达上限后开始刷 `IllegalStateException: Reject hook registration ... already registered 64 times`
（一次会话几百行，日志被大量刷屏）⇒ 所有 hook 现在都带 `sXxxHooked` 旗标。
...
| BZK 的输入法快捷键 | **system_server 的 IMMS** | 只能"旁观"（日志能看到结果），**无法改变"下一个是谁"** ✗ |
...
| `USE_PROVIDER` | `ConfigWatch` | 启用 provider 轮询（在 Gboard 上不可行，见 §9） |

> **教训：实验开关要分层。** 探针的**安装点**原本只看 `DEV_ENTER_TRACE`，关日志时改写一起失效，
> 导致一轮"QQ 还是发了"是**假结论**。**"装不装"与"打不打印"必须分开。**

`EnterFix` 那条 `enter: down gate ...` 是刻意常开的（只有 Enter 会打印，不会造成干扰）：
...
- 按 **Gboard `17.2.2` / `arm64`** 实现并实测；**兼容 `17.2.2` ~ `18.3.1`，
  ...
  其它版本可以尝试，但不保证效果；非 arm64 没有 DexKit native（只影响诊断）。
- 符号归一**只在中文态**（`zh*`）生效；认不出语言时**什么都不做**（fail-safe）。
- 严格模式开启时，**Gboard 屏幕上的语言键 / Shift 中英不再能改语言**（"只接受框架信号"的代价）。
...
- `onStartInput` / `onCreateInputView` 两条框架钩子在 Gboard 上**本来就是死的**（§7），
  相关能力分别由 `setInputView` 与 init 兜底；改动这两处时注意不要依赖它们。
...
所以"首次渲染"**不会**把默认值写进 prefs ⇒ `ConfigSender` 始终读到它自己的 default。
只有用户**手动拨动一次**开关，prefs 才有值、两边才对齐。
...
> **规矩（后续照此执行）**：**默认值只允许有一处来源。**
> 模块侧所有"收到广播之前"的初值，必须等于设置页里那个开关的默认档；
> 同类问题在 §6 已有先例（**配对表没发出去** ⇒ 模块收到 null ⇒ 静默不配对）。
